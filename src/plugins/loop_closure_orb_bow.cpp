...
class LoopClosureOrbBow : public slam::core::LoopClosure {
public:
    LoopClosureOrbBow() = default;
    ~LoopClosureOrbBow() override = default;
    ...
    void initialize(rclcpp::Node::SharedPtr node, const std::string& plugin_name) override {
        ...
        RCLCPP_INFO(logger_, "Initializing LoopClosureOrbBow");

        detector_ = slam::loop_closure::createLoopDetector("orb_bow");

        if (!vocab_path.empty()) {
            if (detector_->loadVocabulary(vocab_path)) {
                RCLCPP_INFO(logger_, "Loaded ORB vocabulary with %zu words", detector_->vocabularySize());
                vocab_loaded_ = true;
            } else {
                RCLCPP_WARN(logger_, "Failed to load vocabulary from %s", vocab_path.c_str());
            }
        } else {
            RCLCPP_WARN(logger_, "No vocabulary path provided for OrbBow. Loop closure disabled.");
        }
    }
...
private:
    ...
    rclcpp::Logger logger_{rclcpp::get_logger("LoopClosureOrbBow")};
...
PLUGINLIB_EXPORT_CLASS(slam::plugins::LoopClosureOrbBow, slam::core::LoopClosure)
